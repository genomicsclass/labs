...
We used this simulated example to demonstrate a simple rotation. This rotation is very much related to PCA.
...
and it maximizes $$(\mathbf{Y}\mathbf{v}_1)^\top (\mathbf{Y}\mathbf{v}_1)$$. This can be viewed as a projection of each row of $\mathbf{Y}$ into the subspace spanned by $\mathbf{v}$. So we are looking for a transformation in which the coordinates show high variability.
...
$\mathbf{v}_1$ is referred to as the _first principal component_ (PC). Also, referred to as  _first eigenvector_, $\mathbf{Yv}_1$
...
The SVD $\mathbf{Y=UDV}^\top$ gives the PCs in columns of $\mathbf{V}$. The first PC is actually close to the last $\mathbf{v}$ we considered:
