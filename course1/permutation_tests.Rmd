...
```{r diff_hist, fig.cap="Histogram of differece between averages from permutations. Vertical line shows the observed difference."}
N <- 12
...
```{r diff_hist_N50, fig.cap="Histogram of differece between averages from permutations for smaller sample size. Vertical line shows the observed difference."}
avgdiff <- replicate(1000, {
    all <- sample(c(control,treatment))
    newcontrols <- all[1:N]
    newtreatments <- all[(N+1):(2*N)]
