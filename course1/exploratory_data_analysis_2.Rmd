...
```{r qqplot_example2, fig.cap="Second example of qqplot. Here we use the function qqnorm which computes the theoretical noraml quantiles automatically."}
qqnorm(x)
qqline(x) 
```
...
```{r qqnorm_of_t, fig.cap="We generate t-distributed data for four degrees of freedom and plot qqplots agains normal theoretical quantiles.",fig.width=10.5,fig.height=10.5}
dfs <- c(3,6,12,30)
mypar(2,2)
for(df in dfs){
  x <- rt(1000,df)
...
```{r scatterplot2, fig.cap="Average son height of each strata plotted against father heights defeining the strata"}
x=(x-mean(x))/sd(x)
y=(y-mean(y))/sd(y)
