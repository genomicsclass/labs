...
The first step is to read in the data. You can do this by downloading the data and using the `read.csv` function as above. Another, more advanced approach, is to download the data directly from within R: 
```{r}
library(downloader)
...
We will include code such as the above throughout the book because it provides a way for readers to cut and paste code that automatically downloads the data.

Once the file is in your home directory you can read in to R:
```{r}
