...
José Iglesias is a professional baseball player. In April 2013, when he was starting his career he was performin rather well:
...
Let's explore the distribution of batting averages for all playes with more than 500 at bats during the previous three seasons:
...
We note that the average player had an average of .275 and the standard deviation of the population of players was 0.027. So we can see already that .450 would be quite an annomally as it is over six SDs away from the mean. So is José lucky or good? If we become convinced that he is luck we should trade him to a team that trusts the .450 observation and thinks this is not just luck.
...
So we started with a _frequentist_ 95% confidence interval that ignored data from other players and summarized just José's data: .450 $\pm$ 0.220. Then we used an Bayesian approach to obtain a posterior probability. This is actually referred to asn empirical Bayes approach because we used data to construct the prior. From the posterior we can report what is called a 95% credible interval by reporting a region, centered at the mean, with a 95% chance of ocurring. In our case this turns out to be: .285 $\pm$ 0.052
