...
Both samples are taken from the non-smokers
```{r}
...
```{r}
N<-100000
clean_dat<-dat[dat$weight!=999,]
medians<-vector(length=N)
for(i in 1:N){
    medians[i]<-median(sample(clean_dat$weight[clean_dat$smoke==1],
                              10))
}
...
```
Try bootstrapping here
```{r}
diff_generator <- function(b=1000,n=50){
  sapply(1:b,function(i){
    smokers_s=sample(smokers,n,replace=TRUE)
    nonsmokers_s=sample(nonsmokers,n,replace=TRUE)
    return(mean(smokers_s)-mean(nonsmokers_s))
    })
}
