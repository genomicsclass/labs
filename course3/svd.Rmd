...
```{r, fig.cap="Second PC plotted against first PC for the twins height data"}
PC1 = s$d[1]*s$v[,1]
PC2 = s$d[2]*s$v[,2]
...
```{r, fig.cap="Entries of the diagonal of D for gene expression data."}
plot(s$d)
```
...
```{r, fig.cap="Percent variance explained by each principal component of gene expression data."}
plot(s$d^2/sum(s$d^2)*100,ylab="Percent variability explained")
```

We can also make cumulative plot

```{r, fig.cap="Cumulative variance explained by principal components of gene expression data."}
...
```{r, fig.cap="Residuals from comparing a reconstructed gene expression table using 95 PCs to the original data with 189 dimensions."}
