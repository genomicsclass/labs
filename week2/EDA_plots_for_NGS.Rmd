...
library("Biobase")
#Count of reads aligned to the first gene for each sample
...
#Explore the sequencing depth issue
totalcount<-order(apply(exprs(bottomly.eset),2,sum))
for(i in order(totalcount)){
    idx <- Y[,i] > -1
    shist(Y[idx,i],unit=0.25,col=i,plotHist=FALSE,
          add=i!=3)
    readline()
    }
