...
biocLite("minfi","IlluminaHumanMethylation450kmanifest","IlluminaHumanMethylation450kanno.ilmn12.hg19")
library(minfi)
```

The first step is to determine the basename of the idat files. Note that for each sample we have two files: one for red and green channels respectively.
...
```{r}
targets<-read.csv("targets.csv",as.is=TRUE)
names(targets)
targets$Basename
```
