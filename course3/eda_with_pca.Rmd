...
```{r predict_sex,fig.cap="Histogram of median expresion y-axis. We can see females and males."}
mypar()
hist(chryexp)
```
...
Year is also very correlated with the first PC. So which variable is driving this? Given the high level of confounding, it is not easy to parse out. Nonetheless, below and in the assessment questions we provide some further exploratory approaches.
...
```{r pc_boxplots, fig.cap="Boxplot of first four PCs stratified by month.", fig.width=10.5,fig.height=10.5}
