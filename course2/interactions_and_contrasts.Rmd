...
Suppose we want to know if the push vs pull difference is *different* for the L3 leg pair vs the L2 leg pair. If we work out the math, this is:
...
type.ss <- sum(sapply(s, function(x) sum((x - mean(x))^2)))
initial.ss - type.ss
```
...
Finally, there is a column which lists the *F value*.
