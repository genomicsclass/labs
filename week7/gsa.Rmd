...
There are various resources on the web that provide gene sets. Most of these have been curated and are composed of genes with something in common. A typical way of grouping them is by pathways (CITE KEGG)
or biological function (GO). A great resource with many curated gene sets can be found here: http://www.broadinstitute.org/gsea/msigdb/index.jsp
...
```{r}
gsets <- getGmt("/Users/ririzarr/myDocuments/teaching/HarvardX/oldgenomicsclass/week7/c1.all.v4.0.entrez.gmt") ##must provide correct path to file
```
...
[cite doi: 10.1093/bioinformatics/btm051]. 
...
The first published approach to performing gene set analysis [CITE http://www.ncbi.nlm.nih.gov/pubmed/?term=11158605] used the Wilcox Mann-Whitney tests discussed in a previous unit. The basic idea was to test if the difference between cases and controls was generally bigger in a given gene set compared to the rest of the genes. The Wilcox compares the ranks in the gene sets are generally above the 
median. 
...
Several methods are based on summaries designed to test shifts in means: for example [http://www.ncbi.nlm.nih.gov/pubmed/16174746], SAFE [http://www.ncbi.nlm.nih.gov/pubmed/15647293], ROAST [http://www.ncbi.nlm.nih.gov/pubmed/?term=20610611], CAMERA [http://www.ncbi.nlm.nih.gov/pubmed/22638577] 

Note that we can construct other scores to test other alternative hypothesis. For example, we can test for changes in variances [http://www.ncbi.nlm.nih.gov/pubmed/?term=20048385] and general changes in distribution [http://www.ncbi.nlm.nih.gov/pubmed/12808457,http://www.ncbi.nlm.nih.gov/pubmed/16199517 ]
...
$$
Vat(\bar{t})= \frac{1}{N^2} \mbox{var}(t_1 + \dots + t_N) = \frac{1}{N^2}(\mbox{var}(t_1)+\dots+\mbox{var}(t_N)) = \frac{1}{N}
$$
...
$$
\begin{eqnarray}
Vat(\bat{t}) &=& \frac{1}{N^2} \mbox{var}((1 \dots 1]) (t_1 \dots t_N)' (1 \dots 1)')
) 
&=& \frac{1}{N^2}(1 \dots 1) 
\begin{pmatrix}
1 & \rho & \dots & \rho & \rho\\
\rho & 1 & \rho & \dots  & \rho \\
\multicolumn{5}{c}{\ddots}\\
\rho &  \rho & \dots & \rho & 1
\end{pmatrix}
(1 \dots 1) '
...
Note that when $\rho$ is positive, the variance is inflated by $\{1 + (N-1) \rho N\}$. Although in general we don't expect all the $\rho$ to be the same within a gene set, a correction factor can still be computed for each gene set that depends on the average $\rho$ in the gene set. The formula can been seen on page 295 here: 
[DOI: 10.1214/07-AOAS146]. The formula for the correction for the Wald statistic is also included. 
