...
Similary to infernece in the context of regression, Machine Learning (ML) studies the relationships between outcomes $Y$ and covariates $X$. In ML we call $X$ the predictors. The main difference between ML and inference is that in ML we are interested in predicting $Y$ using $X$. Statistical models are used, but while in inference we estimate and interprete model parameters, in ML they are mainly a means to an end: predicting $Y$. 
...
In this case we can also approximate the distirbution of $Y$ as normal which implies the mean maximizes the probability density. 

Now imagine we are given more information. We are told the the father of this randomely selected son has height 71 inches (1.2 SDs taller than the average) What is our prediction now? 
...
## Stratification
The best guess is still the expecation, but our strate has changed from all the data to only the $Y$ with $X=71$. So we can stratify and take the average which is the conditional expencations. Out prediction for any $x$ is therefore:
$$
f(x) = E(Y|X=x)
$$
...
The following data are from measurements from replicated RNA. We consider that data used in an the MA-plot ($Y$=log ratios and $A$=averages) and take down-sample in a way that balances the number of points for different stratas of $A$:
...
Note that linear regression is biased does not capture the aparent curvature in $f(x)$:
...
Most of ML is concerned with cases with more than one predictor. For illustration purposes we move to a case in whch $X$ is two dimensional and $Y$ is binary. We simulate a situation with a non-linear relationship using an example from Hastie, Tibshirani and Friednman's book. In the plot below we show the actual values of $f(x_1,x_2)=E(Y|X_1=x_1,X_2=x_2)$ using colors
...
We will review two speicif ML techniques. First, we need to review the main concept we use to evaluate the performance of these methods. 
...
You will notice that the test and train set have similar global properties as they were generated by the same random varialbes (more blue towards the bottom right) but are, by construction, differnet. 
```{r}
mypar(1,2)
plot(x,pch=21,bg=cols,xlab="X1",ylab="X2",xlim=XLIM,ylim=YLIM)
...
A first naive approach to thi ML problem is to fit a two varible linear regression model:
...
Now we can obtain a plot showing where we predict 1s and where we predict 0 as well as the boundary:
```{r}
colshat <- yhat
...
We can also use the `predict` function to obtain predicted values for our test set. Note that nowhere do we fit the model on the test set.  
```{r}
##prediction on test
...
Note that the error rates in the test and train sets are quite similar. Thus do not seem to be overtrainig. This is not surprising as we are fitting a 2 parameter model to 400 data points. However note that the boudary is a line. Because we are fitting plane to the data, there is no other option here. The linear regression method is too rigid. The rigid makes it stable and avoids over training but it also keeps the model from adpating to the non-linear relationship between $Y$ and $X$. We saw this before in the smoothing section. The next ML technique we consider is similar to the smoothing techniques described before.

# K-nearest neighbor

K-nearest neighnors (kNN) is similar to bin smoothing but it is easier to adapt to multiple dimensions. Basically for any point $x$ for which we want an estimate we look for the k nearest points and then take an avarege. We can now control flexibilty through  $k$. Here we comapre $k=1$ and $k=100$.
...
Here is a comparison of the test and train set errors for various values of $k$. We also include the error rate that we would make if we actaully knew $\mobx{E}(Y|X_1=x1,X_2=x_2)$ referred to as _Bayes Rule_
```{r}
###Bayes Rule
yhat <- apply(test,1,p)
