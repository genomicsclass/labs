...
## Note: this script will crash in RStudio, but will work in Terminal
## tested on Mac OS X Snow Leopard May 9, 2014
...
genomics data visualization app. To plot your data there we use the `epivizr` package. Although the package is already part of `Bioconductor`
you can make sure you get the latest version by installing as follows:

```{r}
# biocLite(c("antiProfilesData","hgu133plus2.db"))
# install_github("epivizr-release", "epiviz")
library(epivizr)
```

We assume you already ran the `methylation` lab and have in your environment the following objects:
...
Let's now browse the genome in order through the top 10 found regions in order (by area):
