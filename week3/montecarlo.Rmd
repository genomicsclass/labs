...
We illustrate this with a simple example related to the central limit theorem. In the inference module we used a sample size of 10 men and 10 women to predict the, on average and in the population in question, men are taller than women. We used the central limit theorem to approximate the distribution of the t-statistic as normal with mean 0 and standard deviation 1. But is 10 large enough to use this approximantion? Let's use a monte carlo simulation to corroborate. Below is the code we used to obtain random sample and then the difference of 3.30
...
print(mean(men)-mean(women))
```
But a different random sample give us a different answer
```{r}
for(i in 1:10){
  men=sample(dat$Height[dat$Gender==1],10)
  women=sample(dat$Height[dat$Gender==0],10)
  cat(signif(mean(men)-mean(women),2),", ",sep="")
  }
cat("\n")
```
...
```{r}
ttests <- sapply(1:1000,function(i){
  men=sample(dat$Height[dat$Gender==1],10)
  women=sample(dat$Height[dat$Gender==0],10) 
  return((mean(men)-mean(women))/sqrt((var(men))/10+var(women)/10))
})
```
With 1,000 simulated ocurrences of this random variable wecan now get a gimplse of it's distribution
```{r}
hist(ttests)
```
...
ttests <- sapply(1:1000,function(i){
  men=sample(dat$Height[dat$Gender==1],10)
  women=sample(dat$Height[dat$Gender==1],10) 
  return((mean(men)-mean(women))/sqrt((var(men))/10+var(women)/10))
})
...
```{r}
ttests <- sapply(1:1000,function(i){
  men=sample(dat$Height[dat$Gender==1],3)
  women=sample(dat$Height[dat$Gender==1],3) 
  return((mean(men)-mean(women))/sqrt((var(men))/3+var(women)/3))
})
...
qqplot(qt(qs,df=2*3-2),ttests)
abline(0,1)
```
The t-distribution is a much better approximation in this case

### Simulating the observations

In the previous section we sampled from the entire population. In many cases we don't have access to data from the entire population. In these cases we can simulate that data as well. For the case of heights we could use 
```{r}
menheights=rnorm(25000,69,3)
```
and repeat the entire excercise.

*Homework*:
1) How different are the normal(0,1) and t-distribution when degrees of freedom are 
18? How about 4?
2) For the case with 10 samples, what is the distribution of the sample median height for me? How does the mean, median of the distribution compare to the population median? 
3) Repeat the code above but simulated income for men and women. Is the median income the same? is the mean income the same?

## Permutations
Suppose we have a situation in which non of the standard statistical tools apply. We have computed a summary statisitic, such as the difference in mean, but do not have a useful approximation such as that provided by the CLT. In practice, we do not have access to all values in the population so we can't perform a simulation as done above. Permutation can be useful in these scenarios. 
...
avgdiff <- sapply(1:1000,function(i){
    all=sample(c(men,women))
    menstar=all[1:10]
    womenstar=all[11:20]
  return(mean(menstar)-mean(womenstar))
})  
hist(avgdiff)
abline(v=obs)
print(mean(abs(avgdiff)>obs))
```
Note that the observed difference is larger than 97.7% of the differences obtained after simulation.

Points: Permuations sill have assumptions: samples are assumed to independent. If we have few samples we can't do permutations. Permutations result in conservative p-values since, if there is s a real differences, some of the permutations will be unblanced and will contain signal.
