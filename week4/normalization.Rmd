...
This Dilution dataset has six sets of five technical replicats. The six sets differ in the concentration of RNA. The RNA was diluted 5 times so that 1/2 as much RNA was hybridized in each set. We start by showing  data from five technical replicates. We show boxplots and densities
```{r,fig.width=7}
library(rafalib)
library(affy)
...
To undersand the downstrea consequences of not normalizing we will use the spike-in experiment used in previous units.
```{r,fig.width=7}
library(SpikeIn)
data(SpikeIn95)
spms<-pm(SpikeIn95)
...
for(i in 1:10) ##show the first 10
