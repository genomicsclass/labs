...
ttests <- sapply(1:1000,function(i){
  smokers=sample(dat$bwt[dat$smoke==1],10)
  nonsmokers=sample(dat$bwt[dat$smoke==0],10)
  return((mean(smokers)-mean(nonsmokers))/sqrt((var(smokers))/10+var(nonsmoker)/10))
})
```
...
```{r}
ttests <- sapply(1:1000,function(i){
  smokers=sample(dat$bwt[dat$smoke==1],3)
  nonsmokers=sample(dat$bwt[dat$smoke==0],3)
  return((mean(smokers)-mean(nonsmokers))/sqrt((var(smokers))/3+var(nonsmokers)/3))
})
...
18? How about 4?
2) For the case with 10 samples, what is the distribution of the sample median wight for smokers? How does the mean, median of the distribution compare to the population median? 
...
avgdiff <- sapply(1:1000,function(i){
    ...
    smokersstar=all[1:10]
    nonsmokersstar=all[11:40]
