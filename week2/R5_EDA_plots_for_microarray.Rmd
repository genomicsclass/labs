...
```{r}
shistogram <- function(z,unit,plotHist=TRUE,add=FALSE,
  ...
  if (plotHist) {
    ...
    plot(h, col="grey",main=main,xlab=deparse(substitute(z)),
              ylim=ylim,xlim=xlim)
  } else {
    ymax <- ymax
    ...
    if (!add) {
      plot(0,0,type="n",xlim=xlim,ylim=ylim,
           xlab=deparse(substitute(z)),ylab="Frequency",main=main)
    }
  }
  lines(d$x, d$y*n*unit, lwd=3, col=lineCol)
  if (!add) {
    arrows(min(z) - unit, ymax, 
           min(z), ymax,
...
shistogram(x,unit=2)
shistogram(x,unit=.5)
shistogram(x,unit=.5,plotHist=FALSE,xlim=c(-4,7))
y <- rnorm(500,2,1)
shistogram(y,unit=.5,plotHist=FALSE,add=TRUE,lineCol="firebrick")
```
