...
split is a really useful function for breaking up a vector into groups defined by a second vector, typically a factor. Here we split the weights into a list divided by sex. We can then use the lapply function to calculate the average of each element of the list, which are vectors of weights.
...
A new library dplyr can accomplish the same task, and can be extended to many other more complicated operations. Here is the same task done with group_by and summarise:
