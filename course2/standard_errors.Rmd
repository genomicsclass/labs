...
```
By making qq-plots, we see that our estimates are approximately nomral random variables:
...
We have shown how we can obtain standard errors for our estimates. But, as we learn in the first chapter to perofrm inference we need to know the distribution of these random variables. The reason we went through the effort to compute the standard errors is becasue the CLT applies in linear models. If $N$ is large enough, then the LSE will be normally distributed with mean $\boldsymbol{beta}$ and standard errors as described. 
