...
We begin by loading the tissue gene expression data:
...
We can perform hierarchical clustering based on the distances defined above, using the `hclust` function.
...
Keep in mind that hierarchical clustering does not define specific clusters, but rather defines a dendrogram. To define clusters we need to "cut the tree" at some height and group all samples that are within that split into different groups below that line. We use 120 as an example:
