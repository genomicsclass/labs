...
The design matrix is the matrix $X$ that we have seen in lecture. For fitting linear models in R, we will directly provide the formula to the `lm` function. However, using `model.matrix` will show you what matrix will be created interally within `lm`, connecting the results of the linear model to the equation $y = X \beta + \varepsilon$ that we have seen in the lectures.
...
Let's try an example. Suppose we have two groups, 1 and 2, with two samples each. We should first tell R that these values should not be interpreted numerically, but as different levels of a *factor*. We can then use the paradigm `~ x` to say, model on the variable `x`:
...
What happens if we don't tell R that `x` should be interpreted as a factor?
...
Note, this is **not** the design matrix we wanted, and the reason is that we provided a numeric variable to the `formula` and `model.matrix` functions, without saying that these numbers actually referred to different groups. We want the second column to have only 0 and 1, indicating group membership.
...
This group now fits a separate coefficient for each group. We will explore this more later on.
...
Such a linear model includes an intercept, a term for `x` and a term for `y`. We would say that this linear model accounts for differences in both the x and y variables.

We assume in the above linear model, that the effect of the x and y variables are simply additive. Being in group 2 and group b is equal to the difference between 2 and 1 and the difference between b and a. 

Another model is possible which fits and additional term, which encodes the potential interaction of x and y variables. We will cover interaction terms in depth in later scripts.
