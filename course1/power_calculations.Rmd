...
Whenever we perform a statistical test we aware that we may make a
...
p-values have what might be considered an arbitrary nature when the 
if the null hypothesis is not true and therefore
...
the sample size (supposing that we have an infinite population to draw
...
alternative hypothesis, nlike the t-statistic, which is guaranteed to
