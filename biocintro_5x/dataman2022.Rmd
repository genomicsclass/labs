...
if (!interactive()) opts_chunk$set(fig.path=paste0("figure/", sub("(.*).Rmd","\\1",basename(knitr:::knit_concord$get('infile'))), "-"))
...
As of July 10 2022 the cloud resources are inaccessible.  They
may come back on line, or may be discovered by search.
```{r lkldb, eval=FALSE}
library(ldblock)
sta = stack1kg()
sta
...
JULY 10 2022 - Cloud resources currently inaccessible, code chunk unevaluated.

```{r doorm, cache=TRUE, eval=FALSE}
library(erma)
...
gbm = curatedTCGAData("GBM", c("Mutation", "CNASNP", "RNASeq2GeneNorm",
   "Methylation_methyl27-20160128"), dry.run=FALSE, version="2.0.1")
gbm
```
