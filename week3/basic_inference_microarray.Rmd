...
e <- maPooling
head(pData(e))
#pData(e) has rows for microarrays and columns for experiments. Various combinations of pooling

# install_github("rafalib","ririzarr")
library(rafalib)
mypar()

#flip the image and rotate
flipt <- function(m) t(m[nrow(m):1,])
#Keep the orientation of the original matrix, with 1 plotted as a black square
myimage <- function(m,...) {
...
#locate rows that have single black block
...
#remove technical replicates (which have tr in their names)

individuals <- individuals[-grep("tr",names(individuals))]

es <- e[,individuals]#a smaller matrix with less black blocks
myimage(as.matrix(pData(es)),col=c("white","black"))

#turn column names 'a', 'b' into 0, 1
#add an attribute to S4 class?
...
The exprs data have been normalized
...
# biocLite("genefilter")
library(genefilter)
tt <- rowttests(exprs(es), es$group)#faster way to compute t-statistics along the row
head(tt)
head(tt,1)

mean(exprs(es)[1,es$group == 0]) - mean(exprs(es)[1,es$group == 1]) 

# Alternatively, splited<-split(exprs(es)[1,],es$group); mean(splited$'0')-mean(splited$'1')
simple.t <- t.test(exprs(es)[1,] ~ es$group, var.equal=TRUE)
simple.t$p.value
#Not as fast as rowttests
#Try a slow way
tests<-apply(exprs(es)[1:10,],1,function(x) {
    samples<-split(x,es$group)
...
#ANother way 
# stripchart(split(exprs(es2)[1,],es2$group),vertical=T, method="jitter",col=c(1,2),main="Three samples per group", xlab="Group",ylab="",pch=15)
...
#give artifical expression levels
y <- 1:6
x <- es2$group
...
# Because the ranks are identical, wilcox is not sensitive 
# rank(1:6)
# rank(c(1:3,11:13))

```
Non-parametric methods are robust against outliers, but have disadvantage when the sample size is small
...
#group 1 vs. group 0, group 0 as the intercept
design
#fit a linear model for each row
fit <- lmFit(es, design)
class(fit)#a MArrayLM class
names(fit)
head(coef(fit))
tt[1,]#difference of mean is the same as the coeff for es$group1
...
fit$t[1,]
tt[1,]
#eBayes does not give the same t test values as those given by lmfit
plot(-1 * tt$statistic, fit$t[,2],
     xlab="rowttests", 
     ylab="eBayes t")
abline(0,1,col="red",lwd=3)

#list the top ranked genes 
