...
dat_by_gender<-split(dat,as.factor(dat$Gender))
men<-dat_by_gender[[1]][sample(1:nrow(dat_by_gender[[1]]),10),"Height"]
women<-dat_by_gender[[2]][sample(1:nrow(dat_by_gender[[2]]),10),"Height"]
stripchart(list(men,women),vertical=TRUE,method="jitter",col=c(1,2), pch=15,group.names=c("men","women"))
```
```{r}
men_height<-dat_by_gender[[2]]$Height
sample_means<-NULL
#Kind of animation
for(i in 1:100){
    sample_means<-c(sample_means,mean(sample(men_height,10)))
    breaks=seq(from=62,to=77,by=0.2)
    hist(sample_means,breaks=breaks,ylim=c(0,200))
    readline()
    dev.off()
}
```
...
Volcano plot:
X axis
Effect-size: enumerator of t statistics i.e. log ratio of average gene expression
