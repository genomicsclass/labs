...
Many of the models we use in data analysis can be presented using matrix algebra. We refer to these tpyes of models as _linear models_. Note that "linear"" here does not refer to lines but rather linear combinations. The representations we describe are conveninent because we can write models more succintly and we have the matrix algebra mathematical machinery to facilitate computation. In this chapter we will describe in some detail how we use matrix algebra to represent and fit.
